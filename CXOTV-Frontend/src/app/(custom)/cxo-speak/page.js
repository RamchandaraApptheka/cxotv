    ...
    const fetchCXOTalkData = async (page = 1) => {
        try {
            ...
            const response = await fetch(
                `${BACKEND_URL}/api/news?populate=*&sort=publishedAt:DESC&filters[$or][0][categories].data[name][$eq]=CXO%20Speak&filters[$or][1][subcategories].data[name][$eq]=CXO%20Speak&pagination[page]=1`
            );
