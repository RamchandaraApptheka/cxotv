    ...
    const fetchCXOTalkData = async (page = 1) => {
        try {
            ...
            const response = await fetch(
                `${BACKEND_URL}/api/news?populate=*&sort=publishedAt:DESC&filters[$or][0][categories].data[name][$eq]=IoT&filters[$or][1][subcategories].data[name][$eq]=IoT&pagination[page]=1`
            );
